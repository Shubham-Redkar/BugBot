...
async def _check_required_field_validation(page, form, url, idx, issues):
    try:
        ...
        if await required.count() == 0:
            save_path, url_path = get_screenshot_path("required_field", idx)
...
async def _check_email_validation(page, form, url, idx, issues):
    try:
        ...
        if field_type != "email" and not pattern:
            save_path, url_path = get_screenshot_path("email_validation", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_password_validation(page, form, url, idx, issues):
    try:
        ...
        if not minlength and not pattern:
            save_path, url_path = get_screenshot_path("password_validation", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_missing_form_labels(page, form, url, idx, issues):
    try:
        ...
        if count > 0:
            save_path, url_path = get_screenshot_path("missing_labels", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_insecure_form_submission(page, form, url, idx, issues):
    try:
        ...
        if action and urljoin(url, action).startswith("http://"):
            save_path, url_path = get_screenshot_path("insecure_form", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_broken_images(page, url, idx, issues):
    ...
    try:
        ...
        if broken_srcs:
            save_path, url_path = get_screenshot_path("broken_images", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_missing_alt_text(page, url, idx, issues):
    ...
    try:
        ...
        if count > 0:
            save_path, url_path = get_screenshot_path("missing_alt", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_page_title(page, url, idx, issues):
    """Flag pages with no <title> or a blank one."""
    try:
        title = await page.title()
        if not title or not title.strip():
            save_path, url_path = get_screenshot_path("missing_page_title", idx)
            await page.screenshot(path=save_path, full_page=True)
            issues.append(
                IssueModel(
                    page=url,
...
async def _check_meta_description(page, url, idx, issues):
    """Flag pages missing a meta description."""
    try:
        count = await page.locator("meta[name='description'][content]").count()
        if count == 0:
            save_path, url_path = get_screenshot_path("missing_meta_description", idx)
            await page.screenshot(path=save_path, full_page=True)
            issues.append(
                IssueModel(
                    page=url,
...
async def _check_console_errors(page, url, idx, issues, console_errors: list):
    ...
    try:
        if console_errors:
            save_path, url_path = get_screenshot_path("console_errors", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_empty_buttons(page, url, idx, issues):
    ...
    try:
        ...
        if count > 0:
            save_path, url_path = get_screenshot_path("empty_buttons", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _check_empty_links(page, url, idx, issues):
    ...
    try:
        ...
        if count > 0:
            save_path, url_path = get_screenshot_path("empty_links", idx)
            await page.screenshot(path=save_path, full_page=True)
...
async def _run_page_checks(page, url, idx, issues, console_errors: list):
    ...
    await _check_page_title(page, url, idx, issues)
    await _check_meta_description(page, url, idx, issues)
