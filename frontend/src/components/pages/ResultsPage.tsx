...
// ── Health arc SVG ────────────────────────────────────────────────────────────
const HealthArc: FC<{ score: number; color: string; status: string }> = ({
  score,
  color,
  status,
}) => {
        ...
        <svg width="100" height="54" viewBox="0 0 100 54">
          ...
          <path
            ...
            style={{
              ...
              transition:
                "stroke-dashoffset 1.2s cubic-bezier(0.34,1.1,0.64,1) 0.3s",
            }}
          />
        </svg>
        <div
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov
          ? `0 0 28px rgba(${rgb},0.15),0 8px 32px rgba(0,0,0,0.5)`
          : "none",
        transform: vis
          ? hov
            ? "translateY(-5px)"
    ...
    >
      <div
        style={{
          ...
          marginBottom: "8px",
          textShadow: hov
            ? `0 0 20px rgba(${rgb},0.9),0 0 50px rgba(${rgb},0.4)`
            : color === "#00F5FF"
  ...
  return (
    <button
      ...
      style={{
        ...
        background: lit
          ? `rgba(${rgb},${isActive ? 0.12 : 0.06})`
          : "transparent",
        color: lit ? `rgb(${rgb})` : "#2a2a2a",
        boxShadow: lit
          ? `0 0 16px rgba(${rgb},${isActive ? 0.28 : 0.14}),0 0 40px rgba(${rgb},${isActive ? 0.1 : 0.05})`
          : "none",
  ...
  return (
    <div>
      ...
      >
        <span
          style={{
            color: hov ? `rgb(${rgb})` : "rgba(255,255,255,0.35)",
            fontSize: "14px",
        ...
        <span style={{ flex: 1 }}>
          <span
            style={{
              ...
              fontSize: "10px",
              letterSpacing: "0.12em",
              color: hov ? `rgb(${rgb})` : "rgba(255,255,255,0.7)",
              marginBottom: "2px",
              transition: "color 0.15s",
            }}
          >
      ...
      ? createPortal(
          <div
            ...
            style={{
              ...
              borderRadius: "8px",
              boxShadow:
                "0 20px 60px rgba(0,0,0,0.8),0 0 0 1px rgba(0,245,255,0.06)",
              overflow: "hidden",
...
const ResultsPage: FC<ResultsPageProps> = ({ results, onReset }) => {
  ...
  const high = resolvedResults.issues.filter(
    (i) => i.severity === "High",
  ).length;
  const med = resolvedResults.issues.filter(
    (i) => i.severity === "Medium",
  ).length;
  ...
  const metrics = [
    {
      label: "NODES SCANNED",
      val: resolvedResults.pages_scanned,
      color: "#00F5FF",
      delay: 100,
    },
    {
      label: "ANOMALIES FOUND",
      val: resolvedResults.issues_found,
      color: "#FFFFFF",
      delay: 175,
    },
  ...
  return (
    ...
    >
          ...
          >
            <span
              style={{
                ...
                animation: "heartbeat-btn 1.5s ease-in-out infinite",
              }}
            />
      ...
      >
          ...
          >
            {sevBars.map(([col, cnt]) => (
              <div
                key={col}
                style={{
                  width: `${resolvedResults.issues_found > 0 ? (cnt / resolvedResults.issues_found) * 100 : 0}%`,
                  height: "100%",
                  background: col,
        ...
        >
          <HealthArc
            score={resolvedResults.health_score}
            color={healthColor}
            status={resolvedResults.health_status}
          />
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        {filtered.map((issue, i) => (
          <IssueCard
            key={`${issue.page}-${issue.issue_type}-${i}`}
            issue={issue}
            index={i}
          />
        ))}
